...
exports.updateNotifications = function (callback, homeyaddress, relaxationdb, relaxationms, relaxationdo) {
    ...
    Object.keys(doorbirds).forEach(function(key) {
        ...
        var optionsdooropen = {
            url: "http://"+ address +"/bha-api/notification.cgi?url="+ dooropenurl +"&user=&password=&event=dooropen&subscribe=1&relaxation="+ relaxationdo +"",
            auth: {
                username: user,
                password: pass
            },
        ...
        request(optionsdooropen, function (error, response, body) {
            if (error) {
                callback(error, false);
            }
        });
...
exports.retrieveHistorySnapshot = function (args) {
    return new Promise(function (resolve, reject) {
        ...
        var options = {
            url: "http://"+ address +"/bha-api/history.cgi?event="+ source +"&index="+ history +"",
            encoding: 'binary',
            auth: {
                username: user,
                password: pass
        ...
        request(options, function (error, response, body) {
            if (error) {
                ...
                fs.writeFile("/userdata/snapshot-history.jpg", body, 'binary', function(err) {
                    if(err) {
                        return reject(err);
                    } else {
                        fs.readFile('/userdata/snapshot-history.jpg', function(err, buf) {
                            return resolve(buf.toString('base64'));
                        });
                    }
                });
...
exports.sendHistoryEmail = function (data, args) {
    return new Promise(function (resolve, reject) {
        ...
        var mailOptions = {
            ...
            text: '',
            html: __('<h1>DoorBird Homey App - History Snapshot</h1><p>This is history snapshot ') + ` ${source} #${history}.</p>`,
            attachments: [ {
...
exports.light = function (args, callback) {
    var address     = args.device.address;
    var user        = args.device.username;
    var pass        = args.device.password;
...
exports.door = function (args, callback) {
    var address     = args.device.address;
    var user        = args.device.username;
    var pass        = args.device.password;
...
exports.ask_door = function (args, callback) {
    Homey.manager('speech-input').confirm(__('Do you want me to open the door?'), function(error, confirmed) {
        if (error) {
            Homey.manager('speech-output').say(__('I did not understand you correctly, please try again'));
            callback(error, null);
        } else if (confirmed) {
            exports.door(args, function (error, result) {
                if (error) {
                    Homey.manager('speech-output').say(__('Something went wrong') + ' ' + error);
                    callback(error, null);
                } else {
                    Homey.manager('speech-output').say(__('OK, the door relay has been triggered'));
                    callback(null, true);
                }
            });
        } else if (!confirmed) {
            Homey.manager('speech-output').say(__('No action has been taken'));
            callback(null, true);
        }
    });
}
