      ...
      .then(body => {
        ...
        fetch('http://'+ address +'/bha-api/favorites.cgi?action=save&type=http&title=Homey%20Motion%20Trigger&value='+ motionurl +'&id=1001', {
          ...
          .then(body => {
            ...
            fetch('http://'+ address +'/bha-api/favorites.cgi?action=save&type=http&title=Homey%20Relay%20Trigger&value='+ dooropenurl +'&id=1002', {
                method: 'GET',
                headers: {'Authorization': 'Basic ' + Buffer.from(username + ":" + password).toString('base64')},
                timeout: 2000
              })
...
exports.createSnapshot = function (address, username, password) {
  return new Promise(function (resolve, reject) {
    fetch('http://'+ address +'/bha-api/image.cgi', {
        method: 'GET',
        headers: {'Authorization': 'Basic ' + Buffer.from(username + ":" + password).toString('base64')},
        timeout: 10000
      })
...
exports.retrieveHistorySnapshot = function (address, username, password, args) {
  return new Promise(function (resolve, reject) {
    fetch('http://'+ address +'/bha-api/history.cgi?event='+ args.source +'&index='+ args.history +'', {
        method: 'GET',
        headers: {'Authorization': 'Basic ' + Buffer.from(username + ":" + password).toString('base64')},
        timeout: 10000
      })
