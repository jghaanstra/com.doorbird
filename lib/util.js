...
exports.updateNotifications = function (homeyaddress, relaxationdb, relaxationdbenable, relaxationms, relaxationmsenable, relaxationdo, relaxationdoenable, callback) {
    ...
    Object.keys(doorbirds).forEach(function(key) {
        ...
        var optionsdooropen = {
            url: "http://"+ address +"/bha-api/notification.cgi?url="+ dooropenurl +"&user=&password=&event=dooropen&subscribe="+ relaxationmoenable +"&relaxation="+ relaxationdo +"",
            auth: {
                user: username,
                pass: password
            },
...
exports.sendSnapshot = function (data, args) {
    return new Promise(function (resolve, reject) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        request(options, function (error, response, body) {
            if (error) {
                ...
                fs.writeFile("/userdata/snapshot-history.jpg", body, 'binary', function(error) {
                    if(error) {
                        return reject(error);
                    } else {
                        fs.readFile('/userdata/snapshot-history.jpg', function(error, buf) {
                            return resolve(buf.toString('base64'));
                        });
                    }
                });
